...
impl Handler for HmacHandler {
    ...
    fn grounding(&self) -> &'static [crate::handlers::grounding::Evidence] {
        ...
        &[Evidence {
            ...
            because: "APC supports HMAC via generate_mac/verify_mac (AWS Payment Cryptography Data \
                      API: generation attributes accept HMAC with a TR31_M7_HMAC_KEY, and the \
                      valid algorithms include HMAC_SHA224/256/384/512 — so the SHA-224 variant is \
                      confirmed available). LQ/LS are therefore a realistic future mapping, gated \
                      only because the previous handler parsed a fabricated layout that does not \
                      match the authoritative wire format (PUGD0537-004 Rev A p.405/407): a 2N hash \
                      id, a byte-length HMAC-truncation field, a length-prefixed inline key, and a \
