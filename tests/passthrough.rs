...
/// Futurex: an unhandled command should be forwarded with its `[AOCCCC;…]`
/// framing intact and the discovery log entry should redact known sensitive
/// parameter codes (`AL` PIN block, `AX`/`BT` key blocks, `BG` BDK) while
...
async fn futurex_unhandled_command_is_forwarded_and_log_redacts_sensitive_params() {
    ...
    let mock = MockHsm::spawn(MockBehavior::Respond(canned_reply.to_vec()), 1).await;
    ...
    let proxy_addr = proxy.addr;
    let frame_for_send = client_frame.to_vec();
    let response = tokio::task::spawn_blocking(move || send_recv(proxy_addr, &frame_for_send))
        .await
        .expect("send_recv");
